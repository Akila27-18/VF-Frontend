...
export default function ChatPanel({ wsUrl, user = "You", receiverId }) {
    ...
    setMessages((prev) => {
      ...
      rawMessages.forEach((msg) => {
        ...
        if (msg.type === "typing") {
          ...
          setTypingUsers((prevTyping) => prevTyping.includes(from) ? prevTyping : [...prevTyping, from]);
  ...
  const handleSend = () => {
    ...
    setMessages((prev) => [
      ...prev,
      normalizeMessage({ id:Date.now().toString(), from_user:user, text:trimmed, created_at:new Date().toISOString() })
    ]);

    sendMessage({ type:"message", payload:{ text:trimmed, receiver_id:receiverId } });
    setText("");
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow p-4">
      ...
      <div ref={scroller} className="h-60 overflow-y-auto space-y-3 mb-3 pr-1">
        ...
        {messages.map((m) => (
          <div key={m.id} className={`max-w-[85%] p-2 rounded-lg ${m.from_user===user?"ml-auto bg-[#FF6A00] text-white":"mr-auto bg-gray-100 text-gray-900"}`}>
            <div className="text-sm break-words">{m.text}</div>
            <div className="text-[10px] opacity-70 mt-1">{m.from_user} • {m.time}</div>
          </div>
        ))}
      ...
      <div className="flex gap-2">
        ...
        />
        <button onClick={handleSend} disabled={!text.trim()} className="px-3 py-2 bg-[#FF6A00] text-white rounded hover:bg-orange-600 disabled:opacity-50">Send</button>
      </div>
    </div>
  );
}
